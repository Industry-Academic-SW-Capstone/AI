...
def get_portfolio_style_vector(stocks_df: pd.DataFrame):
    ...
    stocks_df[feature_columns] = stocks_df[feature_columns].replace([np.inf, -np.inf], np.nan)
...
def analyze_portfolio(request: PortfolioAnalyzeRequest):
    ...
    df_with_names = pd.merge(merged_df, stock_db[['단축코드', '한글명']], on='단축코드', how='left')
    ...
    style_breakdown = sorted(style_breakdown, key=lambda x: x['percentage'], reverse=True)
    ...
    persona_match = []
    sorted_matches = sorted(persona_results.items(), key=lambda item: item[1], reverse=True)
